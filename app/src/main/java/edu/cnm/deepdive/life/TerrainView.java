...
public class TerrainView extends View {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    width = resolveSizeAndState(getPaddingLeft() + getPaddingRight() + width,
        widthMeasureSpec, 0);
    height = resolveSizeAndState(getPaddingTop() + getPaddingBottom() + height,
        heightMeasureSpec, 0);
    ...
    if (terrain != null) {
      ...
      for (int i = 0; i < safeTerrain.length; i++) {
        for (int j = 0; j < safeTerrain[i].length; j++) {
          if (terrain[i][j] !=0) {
            canvas.drawCircle((j + 0.5f) * cellSize, (i + 0.5f) * cellSize,
                cellSize / 2, paint);
          }
        }
      }
